	...
	switch (msg.opcode)
	{
		case HFOP_NONCE:
		{
			...
			for (int i = msg.datalen / 8; i; --i, (data = &data[8]))
			{
				...
				if (unlikely(!work))
				{
					applog(LOG_WARNING, "%"PRIpreprv": Got nonce for undefined work",
					       proc->proc_repr);
					continue;
				}
		...
		case HFOP_STATUS:
		{
			...
			for (int i = 0; i < devstate->cores_per_chip; ++i, (proc = proc->next_proc))
			{
				...
				if (unlikely(!thr->next_work))
					continue;
				
				const uint8_t bits = data[i / 4] >> (2 * (i % 4));
				const bool has_pending = bits & 1;
				const bool has_active  = bits & 2;
			...
			applog(LOG_DEBUG, "%s: STATUS from chipaddr=0x%02x with hdata=0x%04x (isn=%lu) had %d jobs started",
			       dev->dev_repr, (unsigned)msg.chipaddr, (unsigned)msg.hdata, isn, n);
		}
	}
	return true;
}
...
{
	while (hashfast_poll_msg(master_thr))
	{}
...
struct device_drv hashfast_ums_drv = {
	...
	// TODO: .thread_shutdown = hashfast_shutdown,
